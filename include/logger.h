 ...
 * It logs the message to both the console and the system log if the debug flag
 * is set.
 *
 * @param fmt The format string for the log message.
 * @param ... The arguments for the format string.
 ...
 *
 * This macro logs an info message before calling the specified register
 * function. If the function returns ITK_ok, it logs a success message.
 * Otherwise, it retrieves the error message, logs it as an error, frees the
 * allocated memory for the error message, and then jumps to the CLEANUP label.
 *
 ...
 * This macro logs an info message before calling the specified register
 * function. If the function returns ITK_ok, it logs a success message.
 * Otherwise, it retrieves the error message, logs it as an error, and frees the
 * allocated memory for the error message.
 *
 * @param func The register function to be called.
 * @param reg_name The name of the register being logged.
 */
...
/**
 * @brief Log error message and goto CLEANUP with error code
 *
 * This macro logs an error message and stores the error code in the EMH.
 * It then sets the return code to the specified code and jumps to the CLEANUP
 * label.
 *
 * @param message The error message to be logged.
 * @param code The error code to be stored.
 */
#define LOGGER_ERROR_GOTO(message, code)                                                                               \
    do                                                                                                                 \
    {                                                                                                                  \
        LOGGER_ERROR("%s", message);                                                                                   \
        EMH_store_error_s1(EMH_severity_error, code, message);                                                         \
        rcode = code;                                                                                                  \
        goto CLEANUP;                                                                                                  \
    } while (0);
